...
def LinearQuantizeOut(x, bit, adc_mode='original', sub_array=128):
    minQ = torch.min(x)
    delta = torch.max(x) - torch.min(x)
    y = x.clone()
    if delta > 0:
        if adc_mode == 'original':
            stepSizeRatio = 2.**(-bit)
            stepSize = stepSizeRatio*delta.item()
            index = torch.clamp(torch.floor((x-minQ.item())/stepSize), 0, (2.**(bit)-1))
            y = index*stepSize + minQ.item()
        elif adc_mode == 'linear':
            min_val = -sub_array
            max_val = sub_array
            step_size = (max_val - min_val) / (2 ** bit)
            index = torch.clamp(torch.floor((x-min_val) / step_size), 0, 2**bit-1) # 0 ~ 2**adc_bits -1 mapping
